function [data_smoothed] = outlier_detection(data)
% perform outlier detection on the data
% data: input data, which is of size (1, n_channels)
...
for i=1:n_arrays
    ...
    for j=1:64
        ...
        % compute mean and std of data values of adjacent channels
        mean_adj = mean(data_seg(adj_index));
        std_adj = std(data_seg(adj_index));
        % if rms value of the channel is 3*std away from the mean, record it as an outlier
        if (data_seg(j) > mean_adj+3*std_adj) || (data_seg(j) < mean_adj-3*std_adj)
    ...
    for ido=1:size(outlierID,2)
        ...
        data_seg(1,outlierID(ido)) = mean(data_seg(:,adj_index),2);     
    end
